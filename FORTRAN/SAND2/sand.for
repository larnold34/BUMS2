...
2       CONTINUE
      READ (11,FMT=*) IQ,IQ2
      READ (11,FMT=*) MAXITER,CHIFAC,DEV 
      CLOSE (11)
C
...
c      PRINT*,' '
c      CALL SIMANN(M,LAMBDA,MAXEVAL)
C
c      PRINT*,' CURRENT VALUES OF LAMBDAS: '
c      DO 62 I=1,M
c	PRINT*, I, LAMBDA(I)
c 62    CONTINUE
c      PRINT*,' '    
...
C
c      CALL CALCFOUT(LAMBDA,FOUT)
C
c      C2 = 0.0
c      DO 66 I=1,M
c	E(I) = 0.0
c	DO 64 K=1,NB
c	  E(I) = E(I)+B(I,K)*FOUT(K)  
c 64      CONTINUE
c	C2 = C2+((D(I)-E(I))**2)/(S(I)**2)
c 66    CONTINUE
C                                    
c      PRINT*,' CHI SQUARE/MAXIMUM ENTROPY  = ',C2
c      PRINT*,' '                         
C
c      PRINT*,' TOTAL NEUTRON FLUENCE RATE/DEFAULT SPECTRUM  = ',FLUX
c      PRINT*,' '                         
C
...
c 68    CONTINUE
C
c      PRINT*,' TOTAL NEUTRON FLUENCE RATE/SOLUTION SPECTRUM = ',FLUX
c      PRINT*,' '                       
C
c      CALL LETHAR(FI,FOUT,ENBF,N,NB,FIL,FL)
C
C     Re-bin the solution spectrum
C
c      N0M1 = N0 - 1
c      CALL REBINFFL(N0M1,D,S,B,FL,ENBF,ENBZKL,C1,FBDS,FBDSL,M,NB)    
C                 
c      PRINT*,' CHI SQUARE AFTER REBINNING TO THE BIN STRUCTURE OF THE '
c      PRINT*,' DEFAULT SPECTRUM            = ',C1
c      PRINT*,' '
C
c      CALL REBINFFL(NB1,D,S,B,FL,ENBF,ENBR,C3,FBRF,FBRFL,M,NB)    
C                 
c      PRINT*,' CHI SQUARE AFTER REBINNING TO THE BIN STRUCTURE OF THE '
c      PRINT*,' RESPONSE FUNCTION           = ',C3
c      PRINT*,' '
C
c      CALL OUTPUT(D,S,E,EH,ENBF,FI,FOUT,FIL,FL,FBDSL,FBRFL,CZ,C1,UNITS,
c     $ LAMBDA,DH,EBH,C2,NI,M,NB,MP1,RFN,EDSP,ISCF,SCF,FSCF,C3,IQDS,IQBS)
C
...
c      READ*,IQ
     ...
     1,FSCF,IQDS,IQBS,EBH,UNITS,CZ,MAXITER,CHIFAC,DEV)
      END IF
C                                                                  
      END
C
...
C
     ...
     1SCF,FSCF,IQDS,IQBS,EBH,UNITS,CZ,MAXITER,CHIFAC,DEV)

      INTEGER I,J,M,NB,MAXITER,NITER,IQ, CHIFAC
      ...
      PRINT*,' '
c      PRINT*,'  ENTER THE MAXIMUM NUMBER OF ITERATIONS : '
c      READ*,MAXITER
c      PRINT*,' '
c      PRINT*,' CHOOSE A CONVERGENCE CRITERIA: (0 OR 1) :'
c      PRINT*,' 0 - MAXIMUM FRACTIONAL DEVIATION IN ANY BIN ' 
c      PRINT*,' 1 - CHI SQUARE VALUE '
c      PRINT*,' '
c      READ*,IQ
       IQ=CHIFAC
c      IF (IQ .EQ. 0) THEN
c        PRINT*,'  ENTER THE MAXIMUM ALLOWED FRACTIONAL DEVIATION : '
c        READ*,DEV
c        PRINT*,' '
c      END IF
C
