...
from .prt import SAReporter
from .prtvec import VectorPrinter
from .fcn import ObjectiveFunction
from bums2.MAXED.utils.NumberUtils import NumberUtils

class SimulatedAnnealing:
    def __init__(self, m, nb, mm, fi, s, d, omega, flux, t, rt, max: bool = True, max_eval=100000, iprint=1):
        #Problem dimensions and data
        self.M = m #Number of detectors
        self.N = m #Number of variables to optimize
        self.NB = nb #Number of energy bins
        self.MM = mm #Flattened response matrix
        self.FI = fi #Default spectrum
        self.S = s #Measurement uncertainties
        self.D = d #Measured data
        self.OMEGA = omega #SA penalty parameter
        self.FlUX = flux #total fluence

        #SA algorithm parameters
        self.T = t #Initial temperature
        self.RT = rt #Temperature reduction factor
        self.EPS = 1e-6 #convergence toelerance
        self.NS = 20 #Cycles per temperature
        self.NT = 5 #Inner iterations
        self.NEPS = 4 #History length for convergence
        self.MAXEVL = max_eval #Max function evaluations
        self.IPRINT = iprint #Verbosity level
        self.MAX = max #Determines if the SA is maximizing or minimizing

        #Search bounds and step control voectors
        self.LB = [-1e25] * self.N
        self.UB = [1e25] * self.N
        self.C = [2.0] * self.N
        self.X = [0.0] * self.N #current solution
        self.VM = [1.0] * self.N
    ...
    def _initialize(self):
        ...
        #Test the edge cases, IBM370 legacy
        for rval in (175, -185, 100):
            print(f"exprep({rval}) = {self.exprep(rval)}")
        print()
            ...
            for z in range(self.NT):
                for j in range(self.NS):
                    for h in range(self.N):
                        self._inner_step(h, z, j, LNOBDS, NUP, NNEW)
    ...
    def _inner_step(self, h: int, z: int, j:int, LNOBDS: int, NUP: int, NNEW: int):
    ...
    def _propose(self, h: int, z: int, j: int, LNOBS: int) -> Tuple[List[float], bool]:
    ...
    def _accept(self, f_new: float) -> bool:
        #Accept uphill always, else by Metropolis cirterion
        if f_new >= self.F:
            if self.IPRINT >= 3:
                print("POINT ACCEPTED")
            for i in range(self.N):
                self.X[i] = self.XP[i]
            return True
        #Need to look at and potentially move 
